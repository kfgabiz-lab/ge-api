...
## 5. 메인 팝업/교육등록/검색어로그/사이트설정 세션 신규 테이블 (2건)

### 5-1. `training_registration` 테이블 — DDL은 별도 파일에 이미 작성됨

FO 트레이닝 세션 등록(리드 캡처) 저장용. Entity: `com.ge.bo.entity.TrainingRegistration`, 진입 API: `POST /api/v1/fo/training/registrations`.

local은 `ddl-auto: update`로 자동 생성되지만(FK 제약은 자동 생성 안 됨), dev/prod는 `scripts/training_registration.sql`을 그대로 실행할 것 — `CREATE TABLE IF NOT EXISTS training_registration`(컬럼 15개) + FK 2개(`curriculum_id`/`session_id` → `page_data.id`) + 인덱스 2개(`email`, `created_at`) 포함.

### 5-2. `search_keyword_log` 테이블 — DDL 신규 작성 (다른 스크립트에도 없었음, 이번에 추가)

FO 검색어 로그(이력성 append-only, 인기검색어 집계용). Entity: `com.ge.bo.entity.SearchKeywordLog`.

local은 `ddl-auto: update`로 자동 생성되지만, dev/prod(`ddl-auto: validate`)는 수동 실행 필요.
...
CREATE INDEX IF NOT EXISTS "IDX_SEARCH_KEYWORD_LOG_SOURCE_NORM_CREATED"
    ON search_keyword_log (source, keyword_norm, created_at);

COMMENT ON TABLE search_keyword_log IS 'FO 검색어 로그(이력성 append-only) — 인기검색어(Popular Keywords) 집계용';
COMMENT ON COLUMN search_keyword_log.source IS '검색어 출처 — 공통코드가 아닌 앱코드 고정값(DOWNLOAD_CENTER=다운로드센터, UNIFIED_SEARCH=통합검색)';
```
