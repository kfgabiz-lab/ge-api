...
public interface IfCertiMasterRepository extends JpaRepository<IfCertiMaster, IfCertiMasterId> {
     ...
     * 반드시 c.ifResult = 'N'인 행만 갱신한다 — 조건 없이 certi_no+bi만으로 갱신하면 같은 문서의 다른 복합키가
     * quarantineDuplicateKeys()로 이미 'E' 격리된 뒤에도 이 문서가 나중에 성공 처리될 때 그 'E' 행까지
     * 'P'로 덮어써버리는 버그가 있었다.
     */
    @Modifying
    @Query("update IfCertiMaster c set c.ifResult = :ifResult where c.certiNo = :certiNo and c.bi = :bi and c.ifResult = 'N'")
