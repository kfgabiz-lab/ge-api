...
/**
 * FO Contact Us 문의 접수 저장 API
 * - POST /api/v1/fo/contact-us — 폼 제출 내용을 DB에 저장(insert)만 한다
 * - 인증 불필요 (SecurityConfig에서 /api/v1/fo/** permitAll)
 * ※ 기존 CTP 전송용 ContactUsController(/api/v1/public/contact-us)와는 완전히 별개
 */
@RestController
...
public class ContactUsInquiryController {
    ...
    public ResponseEntity<ContactUsInquiryResponse> submit(
        ...
        ContactUsInquiryResponse response = contactUsInquiryService.submit(request, clientIp);
